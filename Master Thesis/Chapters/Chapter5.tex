...
This section provides a statistical analysis of the subjective questionnaire data to evaluate the significance of participant responses. Because the Likert scale is ordinal, the Wilcoxon Signed-Rank test was used for each of the 12 questionnaire items. This non-parametric test assesses whether the median scores differ significantly from the neutral point of 4.0, which is the middle of the 1 to 7 scale. A significant result indicates notable agreement or disagreement among participants. The complete results—including medians, interquartile ranges (IQR), test statistics, and p-values—are summarized in Table~\ref{tab:wilcoxon_results_final_exact}.
